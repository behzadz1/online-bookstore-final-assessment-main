...
def view_cart():
    ...
    return response_with_messages(
        {
            "items": [
                {"title": i.book.title, "qty": i.quantity, "price": i.book.price}
...
def checkout():
    cart = load_cart()
    return response_with_messages(
        {"total": cart.get_total_price(), "items": len(cart.items)}
    )
...
def process_checkout():
    ...
    if payment_method == "card":
        ...
        if (
            ...
            or len(card) < 12
            or not expiry
            or not re.match(r"^(0[1-9]|1[0-2])\/\d{2}$", expiry)
            or not cvv
            or not cvv.isdigit()
            or len(cvv) < 3
        ):
    ...
    if payment_method == "card":
        ...
        if (
            not paypal_email
            or not EMAIL_RE.match(paypal_email)
            or not re.search(r"@.+\.", paypal_email)
        ):
            return redirect(url_for("checkout"))
...
def order_confirmation(order_id):
    ...
    if not o:
        return response_with_messages(
            {"error": "Order not found", "confirmation": False}
        )
    return response_with_messages(
        {
            "order_id": o.order_id,
...
def account():
    ...
    u = users[email]
    history = [
        {"order_id": o.order_id, "total": o.total_amount} for o in u.get_order_history()
    ]
