...
def test_clear_cart_flow(app_client):
    # add something first
    try_post_route(
        app_client, ADD_TO_CART_VARIANTS, data={"title": "1984", "quantity": "1"}
    )
...
def test_login_and_account_flow(app_client):
    ...
    app_client.post(
        "/register",
        data={"name": "T", "email": "t@x.com", "password": "p@ss"},
        follow_redirects=True,
    )
    # login
    r = app_client.post(
        "/login", data={"email": "t@x.com", "password": "p@ss"}, follow_redirects=True
    )
    assert r.status_code == 200
...
def test_logout_redirects(app_client):
    ...
    assert r.status_code == 200
    assert (
        "logged out" in r.get_data(as_text=True).lower()
