...
def test_add_invalid_book_title(app_client):
    route, r = try_post_route(app_client, ADD_TO_CART, data={'title':'Nonexistent Book','quantity':'2'})
    assert r.status_code == 200
    assert re.search(r'book.*not.*found', r.get_data(as_text=True), re.I)

def test_large_quantities_multiple_items(app_client):
    app_client.post('/add-to-cart', data={'title':'1984','quantity':'9999'}, follow_redirects=True)
    app_client.post('/add-to-cart', data={'title':'The Great Gatsby','quantity':'8888'}, follow_redirects=True)
...
def test_remove_nonexistent_item(app_client):
    r = app_client.post('/remove-from-cart', data={'title':'Fake Book'}, follow_redirects=True)
    assert r.status_code == 200
    assert 'removed' in r.get_data(as_text=True).lower() or 'messages_text' in r.get_data(as_text=True).lower()

def test_checkout_partially_valid_card(app_client):
    app_client.post('/add-to-cart', data={'title':'1984','quantity':'1'}, follow_redirects=True)
    r = app_client.post('/process-checkout', data={
        'name':'User','email':'user@example.com','address':'1 St',
        'payment_method':'card','card_number':'4242424242424242','expiry':'','cvv':''
...
def test_blank_registration_fields(app_client):
    r = app_client.post('/register', data={'name':'','email':'','password':''}, follow_redirects=True)
    assert r.status_code == 200
    assert re.search(r'invalid|email|error', r.get_data(as_text=True), re.I)

def test_checkout_after_logout(app_client):
    app_client.post('/register', data={'name':'U','email':'logout@test.com','password':'123'}, follow_redirects=True)
    app_client.post('/login', data={'email':'logout@test.com','password':'123'}, follow_redirects=True)
    app_client.get('/logout', follow_redirects=True)
    r = app_client.post('/process-checkout', data={
        'name':'X','email':'logout@test.com','address':'1 St',
        'payment_method':'paypal','paypal_email':'payer@example.com'
    }, follow_redirects=True)
    assert r.status_code == 200
