...
def test_email_format_validation(app_client):
    ...
    for e in bad_emails:
        _, r = post_register(
            app_client, {"name": "User", "email": e, "password": "p@ss"}
        )
        assert r.status_code == 200
        assert re.search(
            r"(invalid|email)", r.get_data(as_text=True), re.I
        ), f"Should reject {e}"
...
def test_email_case_insensitive_uniqueness(app_client):
    _, r1 = post_register(
        app_client,
        {"name": "User", "email": "CaseUser@example.com", "password": "p@ss"},
    )
    _, r2 = post_register(
        app_client,
        {"name": "User2", "email": "caseuser@EXAMPLE.com", "password": "p@ss2"},
    )
...
def test_password_not_stored_in_plaintext():
    ...
    assert (
        getattr(u, "password", None) != "secret"
